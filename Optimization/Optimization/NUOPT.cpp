...
#line 1 "NUOPT.smp"
		...
		//
		smp_line(__LINE__, __FILE__); VariableParameter Q_pena_ped(name = "Q_pena_ped"); this->Q_pena_ped.setEntity(Q_pena_ped); Q_pena_ped.entryOutput();
		//
		...
		//歩行者がいる時の制約
		//smp_line(__LINE__, __FILE__); (u[Idx] - x_PD[Idx])* (u[Idx] - x_PD[Idx]) + (v[Idx] - y_PD[Idx]) * (v[Idx] - y_PD[Idx]) >= 1.0, Idx;
		...
		obj =
			...
			+ sum(Q_v * (v[Idx_eval] - v_ref[Idx_eval]) * (v[Idx_eval] - v_ref[Idx_eval])//
				...
				+ Q_pena_ped* (vel[Idx_eval] * vel[Idx_eval] / ((u[Idx_eval] - x_PD[Idx_eval]) * (u[Idx_eval] - x_PD[Idx_eval]) + (v[Idx_eval] - y_PD[Idx_eval]) * (v[Idx_eval] - y_PD[Idx_eval]) + 0.0001)) // Dist[Idx_eval]
				, Idx_eval);
		//		
