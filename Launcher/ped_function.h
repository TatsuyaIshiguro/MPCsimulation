	...
	//
		...
		{ //歩行者の動き出し
			...
			if (ped.y_pd <= ped.course_width[0] && ped.y_pd >= (-1.0) * (ped.course_width[0] + 1.5))
			{
				...
				if (ped.y_pd <= 0.5)//1回目の休憩はじめ
				{
					ped.vel_pd = 0.0;
					shareddata->count_stop_num1++;
				}
				if (2.0 < shareddata->count_stop_num1 * T_delta)//1回目の休憩おわり
				{
					ped.vel_pd = ped.vel_pd_start;
				}
				if (ped.y_pd <= -0.5)//2回目の休憩はじめ
				{
					ped.vel_pd = 0.0;
					shareddata->count_stop_num2++;
				}
				if (2.0 < shareddata->count_stop_num2 * T_delta)//2回目の休憩おわり
				{
					ped.vel_pd = ped.vel_pd_start;
				}
			}
		...
		{ //歩行者の動き出し
			...
			if (ped.y_pd <= ped.course_width[0] && ped.y_pd >= (-1.0) * (ped.course_width[0] + 1.5))
			{
				...
				if (ped.y_pd <= 0.2)//1回目の休憩はじめ
				{
					ped.vel_pd = 0.0;
					shareddata->count_stop_num1++;
				}
				if (10.0 < shareddata->count_stop_num1 * T_delta)//1回目の休憩おわり
				{
					ped.vel_pd = ped.vel_pd_start;
				}
			}
